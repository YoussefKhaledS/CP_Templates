struct Node {
    int left = 0, right = 0, val = 0, sum = 0;
    Node(){}
    Node (int v, int x) : val(v), sum(v*x){}
};
struct PST {
    ...
    int set(int idx, int val, int ni, int lx, int rx) {
        int id = create_node();
        if(rx - lx == 1) {
            nodes[id] = Node(nodes[ni].val + val, lx);
            return id;
        }
    ...
    Node get_sum(int l, int r, int ni, int lx, int rx) {
        if(ni == 0) return nodes[ni];
        if(lx >= r || rx <= l)
            return Node();
        if(lx >= l && rx <= r)
        ...
        Node ans = Node();
        merge(ans, get_sum(l, r, nodes[ni].left, lx, mid), get_sum(l, r, nodes[ni].right, mid, rx));
        return ans;
    }
    int get_sum(int l, int r, int version) {
        return get(l, r, version, L, R + 1).sum;
    }
